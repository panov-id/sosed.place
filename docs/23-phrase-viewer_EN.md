...
- It pages through **neighbours' phrases only**, in feed order: newest on top, so the next one is older. The viewer skips offers and tables: an offer has its own "Save" button and menu (screen 17), a table has "sit down" (screen 19). The price is named: paging to the end, a person does not see that an offer or a table stood between the phrases; they stay in the feed.
...
- **Swiping works only on a touch screen** (`pointer: coarse`, `00-mechanics_EN.md` §4): right is like, left is hide; after either, the next phrase (decided 2026-09-15). With a mouse or a keyboard the same results come from the buttons; the left and right arrow keys only page and do nothing to the phrase, so that a mistyped key does not hide a neighbour.
...
- **A phrase that expires while open** fades as in the feed and gives way to the next one with the line "this phrase has ended". Expiry does not ask the viewer's permission.
...
- **On a wide screen** (screen 3, "Wide screen") the viewer opens in the feed column, and the conversations on the right stay in place.

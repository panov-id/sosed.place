...
- **The zone is a diagram with a draggable point (decided 2026-08-28)**, the same instrument as the viewing radius in the feed: the circle moves and stretches, with the place named in words and the distance from your approximate point beside it. There is no map here, as in every face (§4 of the mechanics). The zone knob ties the phrase not to a point but to an area around it. **Five steps: 100 m, 300 m, 1 km, 3 km, 10 km; 300 m by default** (steps introduced 2026-08-31 — a free number of metres became a signal in itself: every phrase has a zone of its own, but people publish from the same place with a similar circle, and a repeated exact value joined one author's phrases; the price is named — whoever wanted 700 m takes a kilometre) (the ceiling was reconciled with the spec on 2026-08-26: this said 2 km while `xor.ad/docs/chat_EN.md` §8.3 computes the coarse bounding box from a 10 km maximum — they must not diverge, and 10 was chosen). Three hundred metres hide the doorway while leaving the block recognisable. It is a privacy setting rather than a visibility one: who sees it is decided by the reader's radius, and the zone need only overlap their circle.
  ...
  **One point, one link** (added 2026-09-14 after the review panel): the same point with the same step on different phrases links them to each other. The line saying so stands here, next to the default zone (screen 10), because a default is what makes such a repeat the norm.
- **A discount is an optional field (settled 2026-08-27).** Under the text, a collapsed line "I'm offering a discount"; expanded, three fields: the value as free text ("−20%", "1+1", "second coffee free"), the **end date and time of the discount** (added 2026-08-29, both required — screen 17) and the conditions ("first ten", "while stocks last"). Filled in, the phrase becomes a **private person's offer**, and a like on it creates a match immediately, without waiting for one back (`xor.ad/docs/offers/SPEC_EN.md`). **The line is labelled "one offer at a time"** (added 2026-09-14): the node refuses a second live offer and a repeat of an earlier text, and saying so before sending costs less than a refusal after.
...
- **A person's very first phrase waits on the name as well**: the name goes into the same queue, and the phrase reaches the feed only when both are accepted. If the name is refused the phrase lies waiting until it is fixed; its 4:20 counts from publication, so waiting costs it no life. While the first one waits, a second cannot be sent. **Since 2026-09-14 this is a general rule:** while one's own phrase is being checked, the next one does not go — otherwise parallel sends would pass the "four per hour" limit before the first verdict (`xor.ad/docs/chat_EN.md` §8.3).

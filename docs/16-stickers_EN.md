...
- **The node does not see which sticker was sent — settled 2026-08-27.** The client puts the sticker's identifier **inside the ciphertext**, under the same conversation key as the text; it fetches the image from the catalogue itself. Otherwise the sticker would travel as a plain field and the node would know a fragment of what two people are saying — the opposite of what §8.13 maintains. **The exception to end-to-end encryption is no longer one — rewritten 2026-09-10.** This used to read "the single exception stays single: the deck and the dice, where the node shuffles and sees the deal". Since 2026-09-09 that is untrue: **the whole board and every move** are outside encryption — otherwise the engine could not keep to the rules (`xor.ad/docs/chat_EN.md` §6). Shuffling a deck was the first and narrowest case, and has become part of a larger one.
  **None of this touches a sticker, which is the point of this paragraph:** the identifier still travels inside the ciphertext, and the node does not know which sticker was sent. What is open is the board, not the replies.
...
- ~~Reporting a sticker is narrowed, not closed~~ — **closed 2026-09-03: a sticker gets a target of its own**. The form gains the kind `sticker` beside the already declared `feed_message`, `offer`, `table_line`, `chat` and `other`, and the snapshot carries **only** the catalogue identifier and the sticker's name — both of which sit in the catalogue openly anyway. Not one reply from the conversation goes into it, and that is not a promise but a consequence of the design: conversations are encrypted on the devices, and there is nothing to take.
  The decision follows one already taken — `table_line` was added the same way when tables appeared. The price is named: the node's database gains one more value in the list of kinds, but not today: stickers are declared beyond the alpha, and a key for a subsystem that does not exist is a number nobody reads.

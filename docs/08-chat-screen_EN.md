...
- **On a wide screen** a conversation opens in the right column, 400 px wide — next to the list from 1030 px, in its place between 900 and 1030 px (screen 3, "Wide screen", decided 2026-09-15).
...
- **The conversation waits for the second person** (the owner's decision of 2026-09-18): the header "waiting for an answer" with the match's remaining time — the name is not declined, it is already in the title, the field alive, replies queued without ✓; the second person's consent delivers the queue and starts the conversation's lifespan; a refusal — the tombstone "the offer is gone". Before consent the replies live only on this device (there is no conversation key yet) and are lost on a device change — the line under the queue says so.
...
- **The peer has stepped away** (`00-mechanics_EN.md` §13) — a label saying **"stepped away"** above the input, the input live, with no time of return and no time of leaving. The label is lifted by the returning person's first message or move in that conversation, not by the span (edited 2026-09-14, screen 20; this said "a line replaces the input" [retired]). This is the only place in the whole product where anyone's presence is reported, and it is allowed because the person declared the state themselves rather than the system inferring it.

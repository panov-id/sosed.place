...
- **The code is shown once and confirmed by typing two groups.** Without the confirmation the "next" button stays inactive: "next" gets pressed unread, and recovery cannot ask afterwards. Groups that do not match — a red line under the fields, "Does not match the code — check it against the paper.", and "next" stays inactive (2026-09-18, frame 9; the text was proposed by the UX lens of the entry panel).
...
- **The registration step's texts — the lens quorum's decision of 2026-09-18:** the consent checkbox "I accept the agreement, the policy and the rules"; the second field's label "repeat the PIN"; the code confirmation — "Written it down? Enter the second and the fourth" above two fields; the reasons for an inactive "next" in words: "next — after consent" (step 1) and "next — once the code is written down" (step 2, code shown); no caption during the skeleton and node errors; the button under a document — "close".

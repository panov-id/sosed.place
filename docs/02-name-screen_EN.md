...
- Registration completes on this step: the browser creates a key pair and the server mints an identity identifier. Registration is scoped to that specific browser — a new browser or a private window creates a new identity, with no transfer or recovery between them.

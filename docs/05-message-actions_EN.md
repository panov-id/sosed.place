...
- **A like takes the card out of the feed — the owner's decision of 2026-09-17.** After the tap the card leaves with a "liked · undo" line for a few seconds, like "hidden · undo"; from then on it lives on screen 25 "My likes", and the like can be taken back there until an offer to talk has come of it (decided 2026-09-15); after that, declining is "not now" on the card (screen 6, `00-mechanics_EN.md` §11). A like on a private author's offer makes the offer at once, so it cannot be taken back. [retired] This said "a like is taken back by a second tap" on the card in the feed.
- **The like on a table card is a bookmark with a counter (2026-09-17):** it does not seat you and makes no offer to talk; everyone seated sees the count, nobody sees who; the table goes to "My likes", from where one sits down at it (screen 19).
...
- **Report.** Immediately removes the message from your feed and adds one to that message's counter; when the counter reaches the threshold — a share of the audience, not a number — the message goes for everyone. **It does not touch the author's quota.**
  ...
  **One form, two outcomes — decided 2026-09-04 after the review panel.** Until this edit a single tap did two things at once: it voted for hiding by threshold and declared itself an Article 16 notice. The legal spec keeps them apart (`xor.ad/docs/dsa/SPEC_EN.md` §1: "a report about a message and a report about an offer are different entities"), and the published terms promise a person the same. Now the screen is built that way too:
  ...
  - **A ticked box promises three things**, and the form says them plainly: the notice gets a number, the acknowledgement arrives immediately and automatically (Art. 16(4)), and the decision with its reason is shown on this device, and by email too if one is left (Art. 16(5)). **You see the decision without email — decided 2026-09-14 (R2):** the number and the acknowledgement are shown at once, and the device keeps a receipt code by which it shows the decision when there is one; a notice still carries no notifier identity (`xor.ad/docs/dsa/SPEC_EN.md` §6). The line by the email field: "clear your browser data and you will not see the decision — leave an email". [retired] "With no email there will be no decision — decided 2026-09-11".

...
- **A revision date under each link** (added 2026-08-29), and, if the text changed after the person **read** it, a note beside it: "changed since".
  **It is held by a read mark on the device — clarified 2026-09-10.** This used to
  ...
  the rules re-accepted silently (`reaccept: silent`; retired 2026-09-15, see below), the node wrote the new row
  into the journal itself, and the journal then stated that the person accepted the
...
- **A revision is signed by a date and a hash of the text — decided 2026-08-29.** Each document carries a date in its header, and the storefront build computes the file's `sha256` and writes a manifest beside the texts. The identity records the pair (date, hash) for each of the three documents; the person is shown the **date**, while the hash is evidence rather than decoration.
  ...
  **The storefront build computes it, and that build already existed — it just computed the wrong thing (clarified 2026-08-29).** `deploy/deploy-landing.sh` derived a revision from the start: it took the latest of the `Last updated` lines of the three English documents and put it into `config.js`. So the revision was never a fourth hand-kept place — but it was derived from a date written by hand. `legal_manifest()` now stands beside it: a pair (date, substance `sha256`) per document plus the re-acceptance policy, shipped as `/legal-manifest.json` and as a `legalDocuments` field in `config.js`.
  **The hash is taken with the date line blanked out**, or a date edit would be indistinguishable from a text edit: the date lives inside the file.
  **And it caught a real lie immediately.** The guidelines were edited on 2026-08-27 while the header announced 13 August — for two weeks the document people accept with a checkbox misdated itself. The date is corrected, and what holds it now is `deploy/check-legal-revisions.py`: text moved without the date, the date without the text, both at once — three separate cases, and the first two stop the deploy and CI. The check was broken by injection and went red on every case.
  **And a fourth case — the translations.** They are not hashed, but they declare their own date, and all twenty-five still stood at 13 August. The dates are raised, and the rule is now mechanical: every language version of a document must date itself with the original's day. Month names are not compared — seventeen languages decline them differently and a table of months would rot faster than it caught anything; the day and the year are digits everywhere, the two languages that write the year first included, and they are what drifts.
...
- **Re-acceptance is the same for all three documents — decided by the owner 2026-09-15 after the final review panel (LAW-14), overriding the decision of 2026-08-29.** Once the terms, the privacy policy or the guidelines change, the identity cannot publish or open a chat until it ticks the checkbox for the new edition, while the feed still reads (screen 11). **The node never writes an acceptance on the person's behalf.** Until this edit the guidelines re-accepted silently [retired]: the node recorded the new revision in `legal_acceptances` itself, and the journal then stated that the person had accepted a text nobody had shown them — evidence false by construction. The reason given on 2026-08-29 — stopping a conversation over rules that restate the mechanics teaches people to press "accept" without reading — is met by the screen rather than by silence: one checkbox for all changed documents, not one per document. **The price:** an edit of the guidelines now stops publishing until the tick, just as an edit of the terms does. **Not yet carried into the build:** the re-acceptance policy is set in `deploy/check-legal-revisions.py` (`DOCUMENTS`) and still gives the guidelines `silent` in `deploy/legal-revisions.json`; it has to become `required`, as for the terms; terms §19 and privacy §13 already say that all three wait for the checkbox.
...
- ~~The build step that computes the hash manifest does not exist yet~~ — **written 2026-08-29** (above), and the check runs in both storefronts' CI.
- **Settled by drawing (2026-09-15).** **What the re-acceptance screen looks like** is not drawn (opened 2026-08-29). It is not screen 2: no name and no age, just the list of documents that changed and one checkbox.

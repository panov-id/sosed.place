...
- **An offer is paged in the full-screen viewer (screen 23, 2026-09-17)** as the same card; a swipe on it only pages. A liked private author's offer leaves the feed for screen 25 "My likes" already as an offer to talk (screen 6); a venue's "Save" is not a like — saved offers live on screen 12.
- The button differs: a venue's card carries **"Save"**, a private author's carries a **like**, and it makes a match at once, after which everything goes as for a phrase: the card, the warning and consent from both (screen 6; edited 2026-09-14: this said "leads to a conversation at once").
- **An offer can be liked without a live phrase of your own — settled 2026-08-27.** An ordinary like requires the liker to have a live phrase (`00-mechanics_EN.md` §11): otherwise no match could ever come of it, since a match counts only while both phrases are alive. On an offer the match is one-sided and the argument does not hold — and the rule without this proviso would cancel the mechanism itself: to collect stools somebody is giving away you would first have to write something of your own. The proviso is written into the spec as well (`xor.ad/docs/chat_EN.md` §8.4).
  **The liker's name goes through the queue before the match — decided 2026-09-14 after the review panel.** With no phrase of their own the name has never been checked, and the match shows it to the offer's author; so the like first sends an unchecked name to the queue, and the match appears only after the verdict. Name refused — no match, and the liker sees a line of their own: "your interest was not sent" (`xor.ad/docs/refusal-wordings_EN.md` §3); the like waits for a corrected name (clarified 2026-09-14 after the review panel; this said "the same text as when a phrase is waiting" [retired] — the liker has no phrase).
...
- **A "…" menu on the card — added 2026-09-04 after the review panel.** Until this edit the screen said complaints hide an offer automatically and never showed where those complaints come from: the legal route was described in the spec, the interface route did not exist. There are three actions, and they are **different** — the difference is named in the menu rather than left for the person to guess:

  - **"The discount was not honoured"** — a complaint under `offers/SPEC_EN.md` §10. There is one subject: the announced discount was not given, or the announcement misled (hidden conditions, "new customers only" in small print). Service quality is **not** a subject and is rejected with an explanation, the venue's contact and a route for a consumer dispute. The complaint is private: the author is shown the text and the date, without the complainant's identity and without the time. Gathered from enough different people, such complaints hide the offer — the very mechanism this screen referred to without showing its source.
  - **"The link goes somewhere else"** — a separate type, and it lives **on the redirect screen** rather than on the card (§10.1): that is where a person sees where they are being sent. Two complaints from different people are enough to kill the link immediately and automatically, **provided both were already posting in the feed before the offer came out — their first publication more than a day old** (decided 2026-09-14; worded as in Terms §10 on 2026-09-15 — this said "provided both have had a publication accepted" [retired]); other complaints go to a moderator at high priority. Otherwise a competitor could kill someone else's redirect from two private windows — an identity costs nothing; the offer stays in the feed and the redirect stops working. This one is never shown to the author.
  - **"Report something illegal"** — an Article 16 notice with `target_kind=offer`, built exactly as on screen 5: a checkbox inside the form, optional name and email, the good-faith confirmation and the §5.1 line. The target is filled in from the card, so the snapshot is taken in time (`xor.ad/docs/dsa/SPEC_EN.md` §4).

  **Why three items here and one for a phrase.** A phrase has two outcomes — a vote and a notice — and a checkbox inside one form separates them. An offer has three subjects, decided in three different ways: the discount by reading the announcement, the link by two independent complaints with no review at all, the illegal by the Article 16 queue. Folding them into one form would mean asking a person for what the system already knows.
...
- **A private author has one offer at a time, and the same text will not go through twice — put into words 2026-09-14.** Both limits stand in the spec (`PRIVATE_ACTIVE_OFFERS=1` and the duplicate check, `xor.ad/docs/offers/SPEC_EN.md` §6.1), but the screen never showed them: the refusal came with no text. Each now has its own line (`xor.ad/docs/refusal-wordings_EN.md` §1), and the "I'm offering a discount" line in the composer says "one offer at a time" in advance.

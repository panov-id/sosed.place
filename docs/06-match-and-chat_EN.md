...
- **A "not now" button — added 2026-08-28.** Declining was possible only by staying silent until the match expired, which made doing nothing the only way to say no. The card now carries two buttons: "talk" and "not now".
  ...
  **The refusal is final after a short undo window** (clarified 2026-08-28, screen 7): for a few seconds the list shows "declined · undo", and the refusal is reversible all that time. After that the card is gone and the like that created it is spent: a new match needs a new phrase.

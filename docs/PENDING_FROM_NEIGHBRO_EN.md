...
- RLS audit of `waitlist` and `unique(waitlist.email)` — at the DB (shared by both faces), no sosed-specific work. The `push_subscriptions` part is dropped: push was cancelled on 2026-08-07. See `db/migrations/` in xor.ad.

...
- **The tenth PIN mistake (added 2026-09-14).** The node's share is burned and this device's correspondence is erased for good (screen 12). The identity survives, but it comes back to this device only with the paper code: recovery mints a new share and a new PIN, and the device starts with an empty history; until then the session is frozen: live phrases are taken down, table seats freed, and only recovery and a new support request work; during an attack on the node code entry may wait up to 15 minutes (refined 2026-09-14 after the review panel; this said "the person sets a new PIN, the node issues a new share" [retired]). Until now this state existed neither here nor on screen 12 — only the warning before it.
...
| Waiting before a PIN attempt | Wait a moment | The next attempt is at HH:MM. The wait grows after the fifth mistake. | — |
| The tenth PIN mistake | Correspondence erased | Ten wrong PINs — the correspondence on this device is gone for good. Your identity is intact — the paper code brings it back. | Enter the code |

...
- **The tenth PIN mistake (added 2026-09-14).** The node's share is burned and this device's correspondence is erased for good (screen 12). The identity survives, but it comes back to this device only with the paper code: recovery mints a new share and a new PIN, and the device starts with an empty history; until then the session is frozen and does nothing on the identity's behalf (refined 2026-09-14 after the review panel; this said "the person sets a new PIN, the node issues a new share" [retired]). Until now this state existed neither here nor on screen 12 — only the warning before it.

# Screen 13 — Moving and recovering the identity

## Purpose

Move the identity to another device **or raise it from the paper code** (decided 2026-08-29: the splash sends both kinds of person here, and separating them across screens would ask a person to choose before they understand the difference). The screen was rewritten on 2026-08-26: it used to hold a list of one identity's devices and a way to connect a second. There are no parallel devices — **an identity has exactly one live session** (`00-mechanics_EN.md` §1), and what remains is a move, not a fleet.
...
- **Recovery from the paper code is this screen's second path (decided
  2026-08-29).** Sixteen characters in four groups, typed on a clean device: the
  ...
  **The difference from a move, said out loud:** a move happens from a live device
  ...
  confirm. So recovery has neither a second side nor two minutes: the code lives
  until the person loses it.
  **Conversations after a recovery are the same and just as silent**, until the
  chat key is reissued (§8.13) — exactly as after a move.
- **The "moving" state (added 2026-08-29).** The code is shown and the person
  leaves the screen: the invitation keeps living its two minutes, and coming back
  shows the same code with the time it has left rather than issuing a new one.
  Issuing a new one would breed live invitations for a single identity.

...
- **The board does not go out by itself — decided 2026-08-29.** A game has ended — the board stays. The engine has seen the end of a round itself since 2026-09-09 (`xor.ad/docs/chat_EN.md` §6), but it does not close the board on it: the board lives inside the conversation, not by the outcome of a game. Edited 2026-09-14: this said "by agreement, since there are no rules… it knows no rules, so it cannot know the end" [retired].
...
- **"Resign" and "offer a draw" are statements, not verdicts (2026-09-09).** The engine does not name the winner of a game or check a draw: it watches turn order, the end of a round and the score (§6 of the canon), not the outcome. "I resign" is a unilateral announcement and the game is over for the people; "draw" is a proposal accepted by agreement. **No result of a game is recorded anywhere:** no history of games, no mark on an identity. **The engine keeps the round score** (above, 3:2), and it goes out with the conversation. Edited 2026-09-14: this said "the engine knows no rules… no score" [retired] — which contradicted the 2026-09-09 decision a few lines up.
...
- The board and the pieces; **whoever's turn it is** moves: the engine holds turn order in every class (decided 2026-09-14; this said "both, at any moment" [retired]).
- A highlight on the piece the other person is dragging right now, so the two do not tug at one piece blindly.
- [retired] ~~A "take turns" toggle~~ — removed 2026-09-14: the engine holds turn order, and the button would do nothing.
...
- **The undo request's buttons are "agree" and "leave as is"; a move the engine refuses returns the piece to its cell with a movement no longer than 150 ms, "Not allowed" appears without movement; the bar "Anya's move" carries no time** (the lens quorum's decision of 2026-09-18).

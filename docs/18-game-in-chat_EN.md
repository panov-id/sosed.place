...
- **In a pair the state stays in transit and is never written to the database:** the node judges as it goes and forgets with the game. The price: restarting the node loses the game. At a table it is the other way round — everything is public there, and the state sits in the database beside the table (screen 19).
